...
function bothAtTarget() {
  const t = makeEngine();
  t.engine.addBoards('A', 1000);          // KAWAII が先に到達 -> カウントダウン取得
  t.engine.addBoards('B', 1000);       // BEAUTIFUL も到達 (奪わない)
  assert.strictEqual(t.engine.countdown.team, 'A');
  return t;
}
...
test('保持側が剥がされたら、1000 を維持している相手へカウントダウンが移る', () => {
  ...
  t.engine.attack('B', 1);              // KAWAII 999, BEAUTIFUL 1000
  assert.ok(t.engine.countdown, 'カウントダウンは止まらない');
  assert.strictEqual(t.engine.countdown.team, 'B');
});
...
test('受け渡しは takenFrom / takenBy 付きで通知される', () => {
  ...
  assert.strictEqual(t.last('countdown:cancel').data.takenBy, 'B');
});

test('相手が 1000 未満なら takenBy は null (ただの解除)', () => {
  const t = makeEngine();
  t.engine.addBoards('A', 1000);
  t.engine.addBoards('B', 500);
  t.engine.attack('B', 1);
...
test('受け取った側が 10 秒維持すればラウンドを取る', () => {
  ...
  t.engine.attack('B', 1);
  t.advance(10000);
  assert.strictEqual(t.engine.getState().score.B, 1);
  assert.strictEqual(t.engine.getState().score.A, 0);
});
...
test('剥がし合いになると受け渡しが繰り返され、どちらも勝ちきれない', () => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    t.engine.addBoards(holder === 'A' ? 'B' : 'A', 1);
    t.engine.attack(holder === 'A' ? 'B' : 'A', 1);
    owners.push(t.engine.countdown.team);
  }

  assert.deepStrictEqual(owners, ['A', 'B', 'A', 'B', 'A']);
  assert.deepStrictEqual(t.engine.getState().score, { A: 0, B: 0 });
});

test('受け渡しの直後に相手も落ちれば、カウントダウンは止まる', () => {
  const t = bothAtTarget();
  t.engine.attack('B', 1);              // BEAUTIFUL へ移る
  assert.strictEqual(t.engine.countdown.team, 'B');

  t.engine.attack('A', 1);                 // BEAUTIFUL も 999 に。KAWAII は 999 のまま
  assert.strictEqual(t.engine.countdown, null);
});
