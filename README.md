...
攻撃ギフト（`Rosa` / 10 ダイヤ / 連打可）だけは、自分の板を増やさず
...
| `gifts.attackGiftIds` | `[8913]` | 相手の板を剥がすギフト（Rosa） |
...
対抗手段は攻撃ギフトで、10 ダイヤの `Rosa` 1 個でカウントダウンを解除できます。
...
テストパネル（`H` キー）の **BGM** 欄で、**2 系統を別々に設定**できます。

| | 用途 |
| --- | --- |
| **配信の音量 / 出力先** | 配信に乗る音。OBS が拾うデバイスを指定します |
| **自分でも聞く** + 音量 / 出力先 | 手元で確認する音。既定では鳴りません |

「出力先の一覧を読み込む」を押すとデバイスを選べるようになります
（ブラウザがマイクの許可を求めますが、これはデバイス名を取得するためだけのもので、
録音はしません）。

使い方の例:

- **配信の出力先** に仮想オーディオデバイス（VB-CABLE など）を選び、OBS でそれを拾う
- **自分の出力先** にスピーカーやヘッドホンを選び、音量を小さめにする

これで、視聴者に届く音量と自分に聞こえる音量を独立して調整できます。

> **OBS だけで済ませることもできます。** ブラウザソースの「詳細音声プロパティ」で
> 「モニターと出力」を選べば、配信に乗せつつ手元でも聞けます。
> 仮想デバイスを入れたくない場合はこちらが手軽です。
...
板を直接動かすボタン・キー（`Q/W/E`、`A`、`I/O/P`、`L`）もそのまま残してあります。

> **2 種類のボタンは通る経路が違います。**
>
> | | 経路 | FEVER 倍率 | チーム所属 | 上限 |
> | --- | --- | --- | --- | --- |
> | **TIKTOK EVENTS** | 実配信と同じ（Router → Session → Engine） | **効く** | 効く | 効く |
> | **板を直接動かす** | エンジンを直接叩く | **効かない** | 関係なし | 効く |
>
> `+50` は「板を 50 枚足す」という指示なので、ポイント → 板の換算に掛かる
> FEVER 倍率の対象外です。**FEVER の効きを確かめたいときは TIKTOK EVENTS 側**
> （`G` キーのギフトなど）を使ってください。
>
> 実配信の経路では、FEVER 中は次のようになります。
>
> ```
> GIFT 20 diamond  →  +40   (通常時は +20)
> LIKE 100         →  +2    (通常時は +1)
> 攻撃ギフト        →  倍率なし (fever.multiplyAttack で変更可)
> ```
...
| 操作 | キー | 消えるもの |
| --- | --- | --- |
| `RESET ROUND` | `R` | 板だけ（スコアとチーム所属は残る） |
| **`RESET ALL`** | `M` | **板・スコア・チーム所属・FEVER のすべて** |
| ページの再読み込み | `F5` | 同上（接続も張り直す） |

**チーム所属まで消えるのが `RESET ALL` の要点です。** 所属が残っていると、
同じアカウントでもう一度合言葉をコメントしても `ALREADY` になってしまい、
参加のテストを繰り返せません。

コンソールからも呼べます。

```js
KVB.reset();                  // RESET ALL と同じ
KVB.session.endSession();     // チーム所属だけ消す
```

tikhub 側は起動したままで構いません。ゲーム画面だけをやり直せます。
...
**3.** 画面に出る欄に **LIVE の URL を貼り付けて「接続」**

```
        TikTok LIVE の URL を貼り付けてください
   ┌────────────────────────────────────┐  ┌──────┐
   │ https://www.tiktok.com/@username/live │  │ 接続 │
   └────────────────────────────────────┘  └──────┘
     短縮 URL (vt.tiktok.com/...) や @ユーザー名 でも繋がります
```

以上です。**コマンドに URL を書く必要も、コンソールを開く必要もありません。**

配信を変えたいときも同じ欄に別の URL を貼るだけです。tikhub は止めなくて構いません。
最初から接続先が決まっているなら `npm start -- <LIVE の URL>` でも起動できます。
...
状態は 3 段階で、画面の出方が違います。

| 画面 | 意味 | 対処 |
| --- | --- | --- |
| 右上に **`TIKTOK 未接続`** | tikhub 自体に繋がっていない | tikhub を起動する |
| **URL の貼り付け欄** | tikhub には繋がっているが、配信先が未指定 | LIVE の URL を貼る |
| **`配信の開始を待っています`** | 指定済み。まだ配信が始まっていない | 配信を始めれば自動で繋がる |
| どれも出ない | 配信に繋がっている | — |

**配信を始める前に URL を入れておけます。** 15 秒ごとに確認して、始まった瞬間に
自動で繋がり、待機表示は消えます。別の配信に変えたいときは
「別の配信を指定する」を押してください。

URL の書き方が間違っているなど本当の失敗のときだけ、欄の下に赤い理由が出ます。
...
```js
KVB.tiktok.connect();                                  // 中継サーバーへ繋ぎ直す
KVB.tiktok.connectLive('@username');                   // 配信を切り替える
KVB.tiktok.live;                                       // 今の配信の状態
```
